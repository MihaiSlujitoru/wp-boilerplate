...
if ( ! class_exists( 'HeroTextPurple' ) ) {
	class HeroTextPurple {
		/**
		 * Hook into WordPress.
		 *
		 * @return	void
		...
		function __construct() {
			// Initialize plugin.
			add_shortcode( 'hero_text_purple', array( $this, 'render_shortcode' ) );
		...
		public function create_shortcode() {
			...
			vc_map( array(
				'name' 			=> __( 'Hero Text Purple', TCM_VC ),				
				'base' 			=> 'hero_text_purple',
		...
		*/
		public function render_shortcode( $atts, $content, $tag ) {
			...
			$output .= 	"<div class='hero-text hero-text-purple vc_row wpb_row vc_row-fluid vc_row-o-equal-height vc_row-o-equal-height vc_row-o-content-middle vc_row-flex ${extra_class}' id='${element_id}'>";
			...
			if( ! empty($hero_subtitle ) && !doing_filter( 'the_excerpt' )) { $output .= 	"<p class='subtitle text-orange'>${hero_subtitle}</p>"; }
			if( ! empty($hero_title ) && !doing_filter( 'the_excerpt' )) { $output .= 	"<h1 class='sub-line sub-line-orange'>${hero_title}</h1>"; }
			$output .= 	"</div>";	
			if( ! empty($content)) {
				$output .= 	"<div class='wpb_wrapper-content'>";
				$output .= 	$content;
				if(!empty($url)) {	
					$output .= 	"<a class='btn btn-orange margin-bottom-0' href='${url}' title='${linkTitle}' target='" . (!empty($target) ? $target : '_self') . "' rel='${rel}'>${linkTitle}</a>";
				}
				$output .= 	"</div>";			
			}				
