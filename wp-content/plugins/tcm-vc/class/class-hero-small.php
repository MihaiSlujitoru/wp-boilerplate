...
if ( ! class_exists( 'HeroSmall' ) ) {
	class HeroSmall {
		...
		function __construct() {
			// Initialize plugin.
			add_shortcode( 'hero_small', array( $this, 'render_shortcode' ) );
		...
		*/
		public function render_shortcode( $atts, $content, $tag ) {
			...
			$output .= 	"<div class='hero-small vc_row wpb_row vc_row-fluid vc_row-o-equal-height vc_row-o-equal-height vc_row-o-content-middle vc_row-flex ${extra_class}' id='${element_id}'>";
			...
			if( ! empty($hero_title ) && !doing_filter( 'the_excerpt' )) { $output .= 	"<h1 class='text-white sub-line sub-line-orange margin-bottom-0'>${hero_title}</h1>"; }
			$output .= 	"</div>";	
