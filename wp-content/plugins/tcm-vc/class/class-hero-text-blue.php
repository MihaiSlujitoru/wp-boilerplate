...
if ( ! class_exists( 'HeroTextBlue' ) ) {
	class HeroTextBlue {
		...
		function __construct() {
			// Initialize plugin.
			add_shortcode( 'hero_text_blue', array( $this, 'render_shortcode' ) );
		...
		*/
		public function render_shortcode( $atts, $content, $tag ) {
			...
			$output .= 	"<div class='hero-text hero-text-blue vc_row wpb_row vc_row-fluid vc_row-o-equal-height vc_row-o-equal-height vc_row-o-content-middle vc_row-flex ${extra_class}' id='${element_id}'>";
			...
			if( ! empty($hero_subtitle ) && !doing_filter( 'the_excerpt' )) { $output .= 	"<p class='subtitle text-blue'>${hero_subtitle}</p>"; }
			if( ! empty($hero_title ) && !doing_filter( 'the_excerpt' )) { $output .= 	"<h1 class='sub-line sub-line-blue'>${hero_title}</h1>"; }
			$output .= 	"</div>";	
			if( ! empty($content)) {
				$output .= 	"<div class='wpb_wrapper-content'>";
				$output .= 	$content;
				if(!empty($url)) {	
					$output .= 	"<a class='btn btn-blue margin-bottom-0' href='${url}' title='${linkTitle}' target='" . (!empty($target) ? $target : '_self') . "' rel='${rel}'>${linkTitle}</a>";
				}
				$output .= 	"</div>";			
			}				
